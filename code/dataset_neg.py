...
class RelationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        y_inputs = [label]
        noise_labels = self.sample_negatives(self.negative_sample, label)
        y_inputs.extend(noise_labels)
        y_encoded_inputs = [l * self.n_sense + i for l in y_inputs for i in xrange(self.n_sense)]
        return torch.from_numpy(features), torch.LongTensor([feature_length]), \
               torch.from_numpy(word_mask).byte(), torch.LongTensor([label]), \
               torch.LongTensor(y_encoded_inputs)
...
class DataSet:
    def __init__(self, opt, model_type):
        ...
        self.valid_loader = DataLoader(valid_data, batch_size=batch_size, shuffle=True, num_workers=n_worker)
